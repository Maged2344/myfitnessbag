...
def process_mfb_excel():
    ...
    # Map existing Order Numbers to their row
    order_number_map = {}
    for row in range(2, delivery_ws.max_row + 1):
        order_num = delivery_ws.cell(row=row, column=col_existing_order_number).value
        if order_num:
    ...
    print(f"[INFO] Matched and deleted {deleted} duplicated rows from 'MFB_Delivery'.")
